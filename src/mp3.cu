...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numARows,
  ...
  __shared__ float cacheA[TILE_WIDTH][TILE_WIDTH];
  __shared__ float cacheB[TILE_WIDTH][TILE_WIDTH]; 
  
  int bx = blockIdx.x;  int by = blockIdx.y;
  int tx = threadIdx.x; int ty = threadIdx.y;

  //@@ In this case the block dimension are (TILE_WIDTH, TILE_WIDTH, 1), so 
  //@@ we could direcly use TILE_WIDTH
  ...
  {
    //@@ Boundary case filter
    if ( row < numARows && t * TILE_WIDTH + tx < numAColumns ){
      cacheA[ty][tx] = A[row * numAColumns + t * TILE_WIDTH + tx];
    }else{
      cacheA[ty][tx] = 0.0;
    }
    if ( t * TILE_WIDTH + ty < numBRows && col < numBColumns ){
      cacheB[ty][tx] = B[col + (t * TILE_WIDTH + ty) * numBColumns];
    }else{
      cacheB[ty][tx] = 0.0;
    }
...
int main(int argc, char **argv) {
  ...
  //@@ It is assumed that there is only one device property
  wbLog(TRACE, "The tile size is ", TILE_WIDTH, " x ", TILE_WIDTH);
